...
const monsterDB = JSON.parse(fs.readFileSync(__dirname + "/datas/monsters.json"));
    ...
    if (field.events.length > 0) {
      ...
      if (event.type === "battle") {
        ...
        if(Math.random()*100<event.percent){
          ...
          //몬스터와 전투
          event = {description : eventManager.data["battle"+monsterType].description + '\n'};
          let enemyStats = monsterDB[monsterType];
          let enemyHP=enemyStats.HP;
          while(enemyHP>0){
            player.incrementHP(-parseInt(enemyStats.str*10/(10+player.def)))
            event.description+= enemyStats.name + "에게 데미지를 입었습니다.\n"
